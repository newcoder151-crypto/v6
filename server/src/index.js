...
[recPath, hlsPath].forEach(p => { if (!fs.existsSync(p)) fs.mkdirSync(p, { recursive: true }); });
...
app.get('/api/health', (req, res) => res.json({
  status: 'ok', version: '2.0.0', service: 'railway-nvr-api',
...
} catch (e) { console.warn('[swagger] Could not load swagger doc:', e.message); }
...
async function start() {
  ...
  if (!ok) {
    ...
    console.error('    3. Check server/.env for DB_HOST, DB_NAME, DB_USER, DB_PASSWORD\n');
    process.exit(1);
  }
  server.listen(PORT, () => {
