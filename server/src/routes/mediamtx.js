...
router.get('/streams', authenticate, async (req, res) => {
  try {
    ...
    res.status(503).json({ error: 'MediaMTX not reachable', detail: err.message, hint: 'Run: bash start.sh' });
  }
});
...
router.get('/cameras', authenticate, async (req, res) => {
  try {
    ...
    } catch { /* MediaMTX may still be starting */ }
...
router.get('/status', authenticate, async (req, res) => {
  try {
    const [paths, version] = await Promise.all([
      axios.get(`${MTX_API}/v3/paths/list`, { timeout: 3000 }).then(r => r.data).catch(() => null),
      axios.get(`${MTX_API}/v3/config/global/get`, { timeout: 3000 }).then(r => r.data).catch(() => null),
    ]);
    res.json({
      online: !!paths,
...
router.post('/sync', authenticate, async (req, res) => {
  try {
    ...
    res.json({ message: 'Sync runs every 15s automatically. Changes will appear shortly.' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
